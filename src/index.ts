...
export type FormattingProps = {
  /**
   * Rows shown per audit — enough to act on without burying the log.
   * @default 5
   */
  maxItems?: number
  /**
...
const formatItems = (
  ...
  { maxItems, maxValueLength }: Required<FormattingProps>
): string[] => {
...
export const logRecommendations = ({
  ...
  const lines = [
    '',
    `───── Lighthouse recommendations: ${label} — ${lhr.finalDisplayedUrl} ─────`,
  ]
